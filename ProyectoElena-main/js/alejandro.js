const usuario = "alejandro";
    ...
    const pesoChart = new Chart(ctx, {
      ...
      data: { 
        labels: [], 
        datasets:[{ label:'Peso Máximo (kg)', data:[], backgroundColor:['#3356ff','#4a70ff','#627eff','#7a94ff','#9bb3ff'] }] 
      },
      options: { 
        responsive:true, 
        maintainAspectRatio:false, 
        scales:{ y:{ beginAtZero:true } }, 
        plugins:{ legend:{ display:false }, title:{ display:true, text:'Progresión de tus levantamientos', color:'#ffffff', font:{ size:16, family:'Poppins' } } }
      }
    });
    ...
    form.addEventListener('submit', e=>{
      ...
      if(!ejercicio || isNaN(peso)) return;
    ...
    modalPesos.addEventListener('click', e=>{ if(e.target===modalPesos) modalPesos.style.display="none"; });

    // Modal Rutina
    const rutinaBox = document.getElementById('rutinaBox');
    const modalRutina = document.getElementById('modalRutina');
    const cerrarModal = document.getElementById('cerrarModal');
    rutinaBox.addEventListener('click', () => { modalRutina.style.display='flex'; });
    cerrarModal.addEventListener('click', () => { modalRutina.style.display='none'; });
    modalRutina.addEventListener('click', e => { if(e.target === modalRutina) modalRutina.style.display='none'; });